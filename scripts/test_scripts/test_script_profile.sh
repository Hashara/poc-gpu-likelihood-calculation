        ...
        for length in "${lengths[@]}"; do
            ...
            for type in "${executable_type[@]}"; do
                ...
                if [ -f "$executable_path" ]; then
                    ...
                    nsys profile --trace=cuda,openacc,nvtx --stats=true -o profile_report $executable_path -s alignment_${length}.phy -te tree.nwk -prefix output_${taxa_size}_${length}_${type}.txt


#		    ncu -o profile_ncu --set full $executable_path -s alignment_${length}.phy -te tree.nwk -prefix output_${taxa_size}_${length}_${type}.txt
                    if [ $? -ne 0 ]; then
